...
@Component
@Slf4j
public class OrderForMasterTask {
    ...
    public void handleCallbackOrder() throws IllegalAccessException {
        ...
        log.info("【任务执行】handleCallbackOrder.start");
...
//            }
            ...
            try {
                ...
                log.info("【任务执行】回调请求消息：notify：{}，req body：{}", notify, reqBody);
                this.redisUtil.pub(String.format("回调请求消息：notify：%s，req body：%s", notify, reqBody));
                resp = HttpRequest.post(notify)
                        .body(reqBody)
                        .execute();
                log.info("【任务执行】回调返回信息： http status： {}， resp： {}", resp.getStatus(), resp.body());
                ...
                if (resp.getStatus() == 200) {
                    ...
                    redisUtil.setRemove(CommonConstant.ORDER_CALLBACK_QUEUE, orderId);
                    log.info("【任务执行】该订单已回调成功，通知url：{}，orderID：{}", notify, orderId);
                }
            } catch (Exception e) {
                log.error("【任务执行】回调失败，notify: {}, resp: {}, err: {}", notify, resp, e);
            }
        }
        log.info("【任务执行】handleCallbackOrder.end");
